...
const CRATE_ROOT = join(MODULE_ROOT, "native", "avif");
const BUILD_DIR = join(MODULE_ROOT, "native", "build", "avif");
const TARGET_DIR = join(BUILD_DIR, "cargo-target");
const CARGO_WASM = join(TARGET_DIR, "wasm32-wasip1", "release", "sinter_avif.wasm");
const WASM_OUT = join(BUILD_DIR, "avif.wasm");
const SOURCE_WASM = join(MODULE_ROOT, "src", "codecs", "avif.wasm");
const DIST_WASM = join(MODULE_ROOT, "dist", "avif.wasm");
...
async function ensureCargo(): Promise<void> {
  ...
  if (!result.ok) {
    throw new Error("cargo is required to build the native AVIF WASM codec.");
  }
}
...
async function buildAvifWasm(): Promise<void> {
  await run("cargo", [
    "build",
    "--locked",
    "--manifest-path",
