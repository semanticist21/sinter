...
const CRATE_ROOT = join(MODULE_ROOT, "native", "bmp");
const BUILD_DIR = join(MODULE_ROOT, "native", "build", "bmp");
const TARGET_DIR = join(BUILD_DIR, "cargo-target");
const CARGO_WASM = join(TARGET_DIR, "wasm32-wasip1", "release", "sinter_bmp.wasm");
const WASM_OUT = join(BUILD_DIR, "bmp.wasm");
const SOURCE_WASM = join(MODULE_ROOT, "src", "codecs", "bmp.wasm");
const DIST_WASM = join(MODULE_ROOT, "dist", "bmp.wasm");
...
async function ensureCargo(): Promise<void> {
  ...
  if (!result.ok) {
    throw new Error("cargo is required to build the native BMP WASM codec.");
  }
}
...
async function buildBmpWasm(): Promise<void> {
  await run("cargo", [
    "build",
    "--locked",
    "--manifest-path",
